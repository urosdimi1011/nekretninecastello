    ...
    <div class="form-container">
        ...
        @foreach ($fields as $is => $f)
            <div class="form-group mb-3">
                ...
                @elseif ($f['type'] === 'number' && $f['name'] === 'cena')
                    {!! Form::text($f['name'], null, [
                        'id' => $f['name'],
                        'class' => 'form-control cena-input',
                        'inputmode' => 'numeric',
                        'pattern' => '[0-9.]*',
                        'autocomplete' => 'off',
                    ]) !!}
                ...
                @elseif ($f['type'] === 'dropdown')
                    ...
                    <x-forms.dropdown :field="$f" :values="$dd ? $dd->getValues() : collect()" :checkedValues="$dd ? $dd->getCheckedValues() : null" :type="$f['tipDropdown']" />
                @elseif ($f['type'] === 'radio')
                    @php
                ...
                @elseif ($f['type'] === 'video_upload')
                    <div class="video-upload-wrapper" data-field="{{ $f['name'] }}">
                        @if (isset($podaci->video) && $podaci->video)
                            <div class="existing-video-box" id="existingVideoBox">
                                <video controls style="width:100%; max-height:300px;">
                                    <source src="{{ $podaci->video->url }}" type="video/mp4">
                                </video>
                                <button type="button" class="video-remove-btn" id="obrisiPostojeciVideo"
                                    data-url="{{ route('obrisiVideoNekretnine', $podaci->id) }}">
                                    Obriši video
                                </button>
                            </div>
                        @endif
